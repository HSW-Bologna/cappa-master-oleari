...
void ili9488_init(void) {
  lcd_init_cmd_t ili_init_cmds[] = {
      ...
      {ILI9488_CMD_POSITIVE_GAMMA_CORRECTION,
       {0x00, 0x03, 0x09, 0x08, 0x16, 0x0A, 0x3F, 0x78, 0x4C, 0x09, 0x0A, 0x08,
        0x16, 0x1A, 0x0F},
       15},
      {ILI9488_CMD_NEGATIVE_GAMMA_CORRECTION,
       {0x00, 0x16, 0x19, 0x03, 0x0F, 0x05, 0x32, 0x45, 0x46, 0x04, 0x0E, 0x0D,
        0x35, 0x37, 0x0F},
       15},
  ...
  while (ili_init_cmds[cmd].databytes != 0xff) {
    ...
    ili9488_send_data(ili_init_cmds[cmd].data,
...
void ili9488_flush(lv_disp_drv_t *drv, const lv_area_t *area,
  ...
  do {
    mybuf =
        (uint8_t *)heap_caps_malloc(3 * size * sizeof(uint8_t), MALLOC_CAP_DMA);
    if (mybuf == NULL)
